"""FastAPI application for clinical-nlp-toolkit."""
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "clinical-nlp-toolkit", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "clinical-nlp-toolkit", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "clinical-nlp-toolkit", "version": "0.1.0", "ready": True,
